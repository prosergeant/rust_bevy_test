...
pub fn play_jump_sounds(mut jump_events: EventReader<JumpEvent>, assets: Res<GameAssets>) {
    for _event in jump_events.read() {
        println!("🔊 Воспроизводится звук прыжка (пока заглушка)");
...
pub fn play_score_sounds(mut score_events: EventReader<ScoreEvent>, assets: Res<GameAssets>) {
